package main

import (
	"testing"

	"github.com/stretchrcom/testify/assert"
)

func TestKeyCreate(t *testing.T) {
	keyIns := NewKeyResp("ssh-rsa  user@server")

	assert.Equal(t, keyIns.PublicKey, "ssh-rsa  user@server", "public Key should not get altered")

	assert.Equal(t, keyIns.Email, keyIns.PopEmail(), "Check for email")

	assert.Equal(t, "f7:ee:58:36:d8:20:38:6c:de:e2:1b:f0:48:ec:08:4c", keyIns.PopFingerPrint(), "Check for fingerprints")

	// Add other tests when everting works
}
